...
이번 화 제목/목표: {json.dumps(ep,ensure_ascii=False)}
...
반복, 같은 감정 재서술, 장황한 풍경, 메타 발언 금지. 행동/대화/갈등/단서/선택으로 전진시켜라.
마지막은 다음 화를 누르게 만드는 구체적 위험/정보/선택으로 끝내라. 본문만 출력.
...
def expansion_prompt(episode,text):
    ...
    return f'''아래 {episode}화 원고는 공백/줄바꿈 제외 {current}자다. {TARGET_VISIBLE_MIN}~{TARGET_VISIBLE_MAX}자로 확장하라.
기존 사건 순서와 결말 훅을 유지하고 반복/재진술/풍경 늘이기 없이 실질적 사건, 대화, 단서, 선택만 보강한다. 원고 전체만 출력.
[원고]\n{text}'''

def contraction_prompt(episode,text):
    current=visible_chars(text)
    return f'''아래 {episode}화 원고는 공백/줄바꿈 제외 {current}자로 길다. 핵심 사건/단서/인과/결말 훅을 유지하고 중복만 덜어 {TARGET_VISIBLE_MIN}~{TARGET_VISIBLE_MAX}자로 압축하라. 새 설정 추가 금지. 원고 전체만 출력.
[원고]\n{text}'''

def repair_prompt(episode,text,issues):
    return f'''한국 상업 웹소설 {episode}화 최종 교정이다. 아래 오류만 최소한으로 고쳐라. 사건/설정/문단순서/결말 훅을 바꾸지 마라. 원고 전체만 출력.
...
def continuity_review(cfg,episode,previous,text):
    prompt=f'''웹소설 연속성 검수자다. 이전 고정 원고와 {episode}화를 비교해 명백한 설정 충돌만 찾는다.
취향/문체/속도는 평가하지 않는다. 인물, 나이, 직업, 부채, 장소, 시간, 사건 결과, 이미 확정된 규칙, 직전 화 훅과의 모순만 지적한다.
반드시 JSON 객체 하나만 출력: {{"issues":[{{"phrase":"현재 화 실제 문구","reason":"충돌 이유"}}]}}. 없으면 {{"issues":[]}}.
{context_text(previous)}\n[현재 {episode}화]\n{text}'''
    last=None
...
def main():
    episode=int(os.environ.get('EPISODE','3'))
    previous,bible,ep,memory=load_context(episode); cfg=config_from_env()
    text=clean(generate(cfg,make_prompt(episode,previous,bible,ep,memory))); generation_passes=1
    expansion_passes=contraction_passes=repair_passes=0
    ...
    while review_now and repair_passes<MAX_REPAIR_PASSES:
        before=text; text=clean(generate(cfg,repair_prompt(episode,text,review_now))); preservation+=preservation_gate(before,text)
        text=apply_lexical_fixes(apply_safe_fixes(text)); repair_passes+=1; review_now=review(cfg,text)
    continuity=continuity_review(cfg,episode,previous,text); lexical_after=scan_lexical(text)
    ...
    (out/f'chapter-{episode}.md').write_text(text,encoding='utf-8')
    report={'episode':episode,'chars_with_spaces':len(text),'chars_without_whitespace':visible_chars(text),'target_chars_without_whitespace':[TARGET_VISIBLE_MIN,TARGET_VISIBLE_MAX],
      ...
      'lexical_preflight_detected':lexical_before,'lexical_preflight_final':lexical_after,'final_review_issues':review_now,'continuity_issues':continuity,'issues':issues,
      'lexical_preflight':'PASS' if not lexical_after else 'BLOCK','independent_reviewer':'PASS' if not review_now else 'BLOCK','continuity_reviewer':'PASS' if not continuity else 'BLOCK','gate':'PASS' if not issues else 'BLOCK','publication_state':'DRAFT_REVIEWED'}
    (out/f'chapter-{episode}-quality.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
