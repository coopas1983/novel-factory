...
(async()=>{
 ...
 const ls=await page.evaluate(()=>({local:Object.keys(localStorage),session:Object.keys(sessionStorage),body:(document.body.innerText||'').slice(0,12000)}));
 ...
 console.log(JSON.stringify({url:page.url(),storageKeys:{local:ls.local,session:ls.session},body:ls.body,responses:seen.map(x=>({url:x.url,status:x.status,body:x.body.slice(0,5000)}))},null,2));
 await browser.close();
