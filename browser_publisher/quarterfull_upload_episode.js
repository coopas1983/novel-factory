...
(async()=>{
 ...
 if(await episodeNode.count()===0){
   ...
   const cmd=`현재 작업 중인 작품 ${TARGET}에서 기존 회차는 절대 수정하지 말고, 원고(출간용) 폴더 아래에 새 문서 ${label}를 생성만 해줘. ${label} 본문은 작성하거나 수정하지 마.`;
   await ai.fill(cmd);
   const send=page.locator('button[aria-label="전송"]');
   if(await send.count()===0) throw new Error('AI_SEND_BUTTON_NOT_FOUND');
